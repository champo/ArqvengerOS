...
\section{Procesos}
    Al comenzar el nuevo trabajo práctico, uno de los primeros desafios era el de implementar un sistema multitarea.
     En un principio, se diseñaron estructuras adecuadas para poder contener toda la información que formaba a un proceso,
      así como los datos necesarios para poder recuperar los estados de un proceso a la hora de ser el proceso ejecutado
     ...
     La jerarquía de los procesos fue modelada de forme similar a la de \textit{Linux} y una vez preparados los datos
      necesarios se utilizó el uso de llamadas como ???? para la creación de estos procesos.
...
\section{Driver Ata}
    ...
    es un aún más interesante la lectura del estado del disco. Esto es necesario para detectar errores en el disco
    ...
    consulta de \{polling} es más rápido que esperar a una IRQ e implementar \textit{polling} es mucho más sencillo.
...
\section{File System}
    ...
    incluyendo nuevas prestaciones en las llamadas a sistema y en nuestra libreria estandar.
    ...
    En una primera instancia nos enrcagamos de adaptar nuevamente nuestras llamadas a sistema y nuestra librería estandar
    ...
    Por otra parte, fue necesarios modificar nuestros procesos para que soporten el manejo de archivos.
